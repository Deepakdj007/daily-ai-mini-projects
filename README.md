...
| [night-scout](night-scout/) | An ambient agent that works while you sleep — nobody prompts it: it wakes on a wall clock through a 23:00-07:00 window, polls HackerNews, arXiv and RSS (all keyless), remembers every item it has already seen, scores a whole wake's finds in one batched call, then drafts a reading-list entry and **parks the run mid-graph** with LangGraph `interrupt()`; the night process exits, and next morning a Streamlit inbox in a different process reads those frozen drafts straight out of the SQLite checkpoints and resumes each one on Approve / Edit / Reject, with a carry-over queue so a per-wake draft budget throttles rather than discards, on free Groq gpt-oss-120b |
| [stock-research-crew](stock-research-crew/) | First CrewAI project in the series — four equity analysts study one NSE stock concurrently (chart, filings, news, sell-side consensus), each scoped to exactly one tool, then a Head of Equity Research joins their briefings into a written research note with a bull case, bear case and where the analysts disagree; parallelism is `async_execution=True` fanning out to a synchronous `context=[...]` join, measured at 9-14s against 20-24s single-file, on keyless yfinance and free Gemini `gemini-3.1-flash-lite` |
| [recall-desk](recall-desk/) | A support desk with three kinds of long-term memory that are genuinely different mechanisms, not one vector store wearing three hats — semantic facts scoped per customer and upserted so a new fact supersedes the one it contradicts; episodic tickets shared across every customer, append-only, scrubbed of identity on write, and retrieved by symptom so one company's outage solves another's next week; and a procedural playbook that is never searched at all because it is always in the prompt, which is why it is capped at six rules, versioned, rollback-able, and edited only when a human gives feedback. Proof is an ablation harness that switches each tier off in turn against an identical seeded store — ablating the read, not the write — with a control probe answerable from the static docs to catch a scorecard that merely rewards longer prompts, and a clean diagonal showing each tier fails in its own way: without semantic it recites the whole price table, without episodic it gives a generic checklist instead of the auto-pause fix, without procedural it opens with an apology and promises a settlement date. LangGraph 1.2 + `SqliteStore`/sqlite-vec + local MiniLM on free Groq `openai/gpt-oss-120b` |
| [safe-sql-agent](safe-sql-agent/) | Chat with a SQL database in plain English without handing an LLM your data — the model drafts a query, then a guard parses it into a syntax tree with sqlglot and judges every node (one statement, SELECT only, allowlisted tables, no denied columns, no bare `*`, no unknown functions) before rebuilding the SQL from the validated tree with a clamped LIMIT, so only code the guard actually saw ever runs; underneath sit a read-only connection, a SQLite authorizer that vetoes each table, column and function as the engine plans the query, and a 3-second abort — with a four-round red team that strips the safety rules from the prompt, watches the model happily write `UPDATE products SET price = 1`, and proves the database is byte-identical afterwards. LangGraph repair loop on free Groq `openai/gpt-oss-120b` |
| [slm-llm-router](slm-llm-router/) | A hybrid router that answers with a 4B model on your own laptop, checks the answer with a verifier that is also local and therefore free, and escalates to a hosted 120B only when the check fails — plus the measurement harness that decides whether any of it was worth doing, because with a free small tier `savings% = 1 − escalation rate`, which means a coin flip reports "90% cheaper" too. The leaderboard runs the cascade against three controls on the same 120 items: an all-SLM floor, a random router at the same routing rate, and an oracle that escalates exactly what the small model got wrong. The cascade earns its keep — 94.2% against random deferral's 81.7% at an identical 32% routing rate — but the honest headline is 56% cheaper, not 90%, because `qwen3.5:4b` is wrong on 26.7% of the traffic and those queries cost 1.37× the average, capping even perfect routing at 63.5%; slice to easy-only traffic and it does hit 97%, which is the real lesson about whose number that is. Exact-match grading with no LLM judge, word-boundary matching that fixes a live substring bug, bootstrap CIs and exact McNemar at n=120, a pre-registered criterion that came back NOT MET, and 40 GSM8K items vendored under MIT so the hard half was not written by the person reporting the result. Ollama `qwen3.5:4b` + free Groq `openai/gpt-oss-120b` |
| [guardrails-layer](guardrails-layer/) | Five layers between a user and a model — a normaliser, a prompt-injection classifier, a policy judge, a sanitiser for retrieved documents, and a PII firewall on the way out — plus the false-positive measurement most demos skip, because a guard that blocks a third of legitimate questions is not a guard. Runs the whole ladder on the weakest model on purpose, and a model-swap control shows the same prompt behaving differently on a different model, which is the point: a prompt is not a control you own. Presidio + Llama Prompt Guard 2 on free Groq `openai/gpt-oss-120b` |
| [context-engineering-layer](context-engineering-layer/) | A composable context assembler — history, window, cap, pin, summarise, retrieve — and the ablation that says which of its six layers earn their place at a fixed 900-token budget. The measurement redid its own budget partway through: comparing the assembler's token count against the server's revealed a per-message framing cost the local tokenizer could not see, so every "we fit in 900 tokens" claim had been optimistic together. Criterion MET at 39% vs 4% on out-of-window needles, with BM25 turning out to find tool output and be blind to prose. Free Groq `openai/gpt-oss-120b` |
| [memory-medic](memory-medic/) | An agent that notices its own memories have gone stale and repairs them — the hard case being a memory that *was* true, which retrieves with the highest score in the store and is stated with complete confidence. A bitemporal store gives every fact two time axes, so a contradiction closes the old window instead of deleting the row and "what did I believe in March" stays answerable; a hygiene sweep then finds the rot nobody reports, because nobody said anything: facts past their volatility half-life, facts grounded in a document that changed while no one was talking, and windows that simply closed. Repairs it is unsure about park on a durable LangGraph `interrupt()` and wait in a Streamlit inbox, which is also how a change to a stable fact never gets applied unattended. Measured with a 40-probe fixture and a five-rung ladder where each rung flips exactly one switch: criterion pre-registered and MET, source drift 0/10 → 10/10 (exact McNemar p=0.00195). Two negatives are reported rather than buried — keeping history changed no answer at all versus deleting it, and the cheapest rung, simply rendering how old a memory is, did most of the work. sqlite-vec + LangGraph 1.2 + local MiniLM on free Groq `openai/gpt-oss-120b` |
