...
--------------------
At n=120 a single item is 0.83 accuracy points. Two strategies differing by
"three points" differ by four items, which may be nothing at all. Reporting a
bare accuracy number at this sample size invites the reader to believe a
precision the experiment does not have.

Every strategy is scored on the same items, so comparisons are paired - which
is far more powerful than treating them as independent samples, and is the only
reason useful conclusions are available at n=120.
"""
